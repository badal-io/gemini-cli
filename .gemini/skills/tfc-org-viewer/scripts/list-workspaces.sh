...
while [[ "$PAGE" != "null" && -n "$PAGE" ]]; do
  RESPONSE=$(curl -s \
    ...
    "https://app.terraform.io/api/v2/organizations/$ORG/workspaces?page%5Bsize%5D=100&page%5Bnumber%5D=$PAGE")
  
  echo "$RESPONSE" | jq -c '.data[]? | {id: .id, name: .attributes.name, project: .relationships.project.data.id}' || true
